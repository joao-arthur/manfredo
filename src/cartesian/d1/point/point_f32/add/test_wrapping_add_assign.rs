...
fn to_bounds() {
    ...
    wrapping_add_assign(&mut p_min, &Point::of(-2.0));
    assert_eq!(p_min, Point::min());

    let mut m_max = Point::of(MAX - 2.0);
    wrapping_add_assign(&mut m_max, &Point::of(2.0));
...
fn out_of_bounds() {
    let mut p_min = Point::of(MIN + 2.0);
    wrapping_add_assign(&mut p_min, &Point::of(-10.0));
    assert_eq!(p_min, Point::of(MAX - 7.0));

    let mut m_max = Point::of(MAX - 2.0);
    wrapping_add_assign(&mut m_max, &Point::of(10.0));
    assert_eq!(m_max, Point::of(MIN + 7.0));
}
...
fn limits_out_of_bounds() {
    ...
    assert_eq!(p_min, Point::of(1.0));

    let mut m_max = Point::of(MAX - 1.0);
    wrapping_add_assign(&mut m_max, &Point::max());
    assert_eq!(m_max, Point::of(-3.0));
}
