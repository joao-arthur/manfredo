...
fn limits_out_of_bounds() {
    ...
    assert_eq!(try_checked_add(&p_min, &PointI::new(i8::MIN, 0)), None);
    assert_eq!(try_checked_add(&p_min, &PointI::new(0, i8::MIN)), None);
    assert_eq!(try_checked_add(&p_min, &PointI::min()), None);

    let p_max = Point::new(MAX - 1, MAX - 1);
    assert_eq!(try_checked_add(&p_max, &PointI::new(i8::MAX, 0)), None);
    assert_eq!(try_checked_add(&p_max, &PointI::new(0, i8::MAX)), None);
