...
fn out_of_bounds() {
    ...
    let mut r_min_x = Rect::new((MIN, MIN + 9.0), (MAX - 9.0, MAX - 9.0));
    assert_eq!(try_checked_inflate_assign(&mut r_min_x), None);
    assert_eq!(r_min_x, Rect::new((MIN, MIN + 9.0), (MAX - 9.0, MAX - 9.0)));

    let mut r_min_y = Rect::new((MIN + 9.0, MIN), (MAX - 9.0, MAX - 9.0));
    assert_eq!(try_checked_inflate_assign(&mut r_min_y), None);
    assert_eq!(r_min_y, Rect::new((MIN + 9.0, MIN), (MAX - 9.0, MAX - 9.0)));

    let mut r_max_x = Rect::new((MIN + 9.0, MIN + 9.0), (MAX, MAX - 9.0));
    assert_eq!(try_checked_inflate_assign(&mut r_max_x), None);
    assert_eq!(r_max_x, Rect::new((MIN + 9.0, MIN + 9.0), (MAX, MAX - 9.0)));

    let mut r_max_y = Rect::new((MIN + 9.0, MIN + 9.0), (MAX - 9.0, MAX));
