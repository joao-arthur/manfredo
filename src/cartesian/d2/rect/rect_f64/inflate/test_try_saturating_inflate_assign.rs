...
#[test]
fn to_bounds() {
    let mut r = Rect::new((MIN + 1.0, MIN + 1.0), (MAX - 1.0, MAX - 1.0));
    assert_eq!(try_saturating_inflate_assign(&mut r), Some(()));
    assert_eq!(r, Rect::largest());

    let mut r_min = Rect::new((MIN, MIN), (MAX - 1.0, MAX - 1.0));
    assert_eq!(try_saturating_inflate_assign(&mut r_min), Some(()));
    assert_eq!(r_min, Rect::largest());

    let mut r_max = Rect::new((MIN + 1.0, MIN + 1.0), (MAX, MAX));
    assert_eq!(try_saturating_inflate_assign(&mut r_max), Some(()));
    assert_eq!(r_max, Rect::largest());

    let mut r_min_x = Rect::new((MIN + 1.0, MIN + 10.0), (MAX - 10.0, MAX - 10.0));
    assert_eq!(try_saturating_inflate_assign(&mut r_min_x), Some(()));
    assert_eq!(r_min_x, Rect::new((MIN, MIN + 9.0), (MAX - 9.0, MAX - 9.0)));

    let mut r_min_y = Rect::new((MIN + 10.0, MIN + 1.0), (MAX - 10.0, MAX - 10.0));
    assert_eq!(try_saturating_inflate_assign(&mut r_min_y), Some(()));
    assert_eq!(r_min_y, Rect::new((MIN + 9.0, MIN), (MAX - 9.0, MAX - 9.0)));

    let mut r_max_x = Rect::new((MIN + 10.0, MIN + 10.0), (MAX - 1.0, MAX - 10.0));
    assert_eq!(try_saturating_inflate_assign(&mut r_max_x), Some(()));
    assert_eq!(r_max_x, Rect::new((MIN + 9.0, MIN + 9.0), (MAX, MAX - 9.0)));

    let mut r_max_y = Rect::new((MIN + 10.0, MIN + 10.0), (MAX - 10.0, MAX - 1.0));
    assert_eq!(try_saturating_inflate_assign(&mut r_max_y), Some(()));
    assert_eq!(r_max_y, Rect::new((MIN + 9.0, MIN + 9.0), (MAX - 9.0, MAX)));
}
...
fn out_of_bounds() {
    ...
    let mut r_x = Rect::new((MIN, MIN + 10.0), (MAX, MAX - 10.0));
    assert_eq!(try_saturating_inflate_assign(&mut r_x), None);
    assert_eq!(r_x, Rect::new((MIN, MIN + 10.0), (MAX, MAX - 10.0)));

    let mut r_y = Rect::new((MIN + 10.0, MIN), (MAX - 10.0, MAX));
    assert_eq!(try_saturating_inflate_assign(&mut r_y), None);
    assert_eq!(r_y, Rect::new((MIN + 10.0, MIN), (MAX - 10.0, MAX)));
}
