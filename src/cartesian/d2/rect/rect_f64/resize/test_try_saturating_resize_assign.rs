...
#[test]
fn bounds() {
    let mut r_min = Rect::of((MIN, MIN), (MIN + 2.0, MIN + 2.0));
    assert_eq!(try_saturating_resize_assign(&mut r_min, 11.0), Some(()));
    assert_eq!(r_min, Rect::of((MIN, MIN), (MIN + 10.0, MIN + 10.0)));

    let mut r_max = Rect::of((MAX - 2.0, MAX - 2.0), (MAX, MAX));
    assert_eq!(try_saturating_resize_assign(&mut r_max, 11.0), Some(()));
    assert_eq!(r_max, Rect::of((MAX - 10.0, MAX - 10.0), (MAX, MAX)));
}
...
fn small_rect_limits() {
    ...
    assert_eq!(r_min, Rect::of((MIN, MIN), (-2.0, -2.0)));

    let mut r_max = Rect::of((MAX - 2.0, MAX - 2.0), (MAX, MAX));
    assert_eq!(try_saturating_resize_assign(&mut r_max, MAX), Some(()));
    assert_eq!(r_max, Rect::of((1.0, 1.0), (MAX, MAX)));
}
