...
#[test]
fn out_of_bounds() {
    let mut r_min_x = Rect::of(MIN, MIN + 2, MIN + 2, MIN + 4);
    assert_eq!(try_wrapping_resize_assign(&mut r_min_x, 5), Some(()));
    assert_eq!(r_min_x, Rect::of(MAX, MIN + 1, MIN + 3, MIN + 5));
...
fn small_rect_limits_out_of_bounds() {
    let mut r_min_x = Rect::of(MIN, MIN + 2, MIN + 2, MIN + 4);
    assert_eq!(try_wrapping_resize_assign(&mut r_min_x, u32::MAX), Some(()));
    assert_eq!(r_min_x, Rect::of(2, 4, 0, 2));
