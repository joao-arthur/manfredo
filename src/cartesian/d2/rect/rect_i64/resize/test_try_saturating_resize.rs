...
fn small_rect_limits() {
    assert_eq!(try_saturating_resize(&Rect::of(MIN, MIN, MIN + 2, MIN + 2), u64::MAX), Some(Rect::of(MIN, MIN, MAX - 1, MAX - 1)));
    assert_eq!(try_saturating_resize(&Rect::of(MAX - 2, MAX - 2, MAX, MAX), u64::MAX), Some(Rect::of(MIN + 1, MIN + 1, MAX, MAX)));
}

#[test]
fn big_rect_limits() {
    assert_eq!(try_saturating_resize(&Rect::of(MIN, MIN, MAX - 1, MAX - 1), u64::MAX), Some(Rect::of(MIN, MIN, MAX - 1, MAX - 1)));
    assert_eq!(try_saturating_resize(&Rect::of(MIN + 1, MIN + 1, MAX, MAX), u64::MAX), Some(Rect::of(MIN + 1, MIN + 1, MAX, MAX)));
    assert_eq!(try_saturating_resize(&Rect::largest(), u64::MAX), Some(Rect::of(MIN, MIN, MAX - 1, MAX - 1)));
}
