...
fn to_bounds() {
    ...
    let mut r_min = Rect::of(MIN, MIN, MAX - 1, MAX - 1);
    assert_eq!(try_saturating_inflate_assign(&mut r_min), Some(()));
    assert_eq!(r_min, Rect::largest());

    let mut r_max = Rect::of(MIN + 1, MIN + 1, MAX, MAX);
...
fn out_of_bounds() {
    ...
    let mut r_x = Rect::of(MIN, MIN + 10, MAX, MAX - 10);
    assert_eq!(try_saturating_inflate_assign(&mut r_x), None);
    assert_eq!(r_x, Rect::of(MIN, MIN + 10, MAX, MAX - 10));

    let mut r_y = Rect::of(MIN + 10, MIN, MAX - 10, MAX);
    assert_eq!(try_saturating_inflate_assign(&mut r_y), None);
    assert_eq!(r_y, Rect::of(MIN + 10, MIN, MAX - 10, MAX));
}
