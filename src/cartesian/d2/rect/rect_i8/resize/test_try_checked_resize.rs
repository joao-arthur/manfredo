...
fn small_rect_limits_out_of_bounds() {
    assert_eq!(try_checked_resize(&Rect::new((MIN, MIN + 2), (MIN + 2, MIN + 4)), u8::MAX), None);
    assert_eq!(try_checked_resize(&Rect::new((MIN + 2, MIN), (MIN + 4, MIN + 2)), u8::MAX), None);
    assert_eq!(try_checked_resize(&Rect::new((MAX - 2, MAX - 4), (MAX, MAX - 2)), u8::MAX), None);
    assert_eq!(try_checked_resize(&Rect::new((MAX - 4, MAX - 2), (MAX - 2, MAX)), u8::MAX), None);
}

#[test]
fn big_rect_limits_out_of_bounds() {
    assert_eq!(try_checked_resize(&Rect::new((MIN, MIN), (MAX - 1, MAX - 1)), u8::MAX), Some(Rect::new((MIN, MIN), (MAX - 1, MAX - 1))));
    assert_eq!(try_checked_resize(&Rect::new((MIN + 1, MIN + 1), (MAX, MAX)), u8::MAX), Some(Rect::new((MIN + 1, MIN + 1), (MAX, MAX))));
    assert_eq!(try_checked_resize(&Rect::largest(), u8::MAX), Some(Rect::new((MIN, MIN), (MAX - 1, MAX - 1))));
}
