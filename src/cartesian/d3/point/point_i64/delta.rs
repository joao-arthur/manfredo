use super::Point;
use crate::cartesian::d3::point::point_u64;
...
pub fn delta_z(p1: &Point, p2: &Point) -> u64 {
    (i128::from(p2.z) - i128::from(p1.z)).unsigned_abs() as u64
}
...
pub fn delta(p1: &Point, p2: &Point) -> point_u64::Point {
    point_u64::Point { x: delta_x(p1, p2), y: delta_y(p1, p2), z: delta_z(p1, p2) }
}
...
mod tests {
    ...
    #[test]
    fn test_delta_x() {
        assert_eq!(delta_x(&Point::new(MIN, 0, 0), &Point::new(MAX, 0, 0)), u64::MAX);
        assert_eq!(delta_x(&Point::new(0, MIN, 0), &Point::new(0, MAX, 0)), 0);
        assert_eq!(delta_x(&Point::new(0, 0, MIN), &Point::new(0, 0, MAX)), 0);
    }

    #[test]
    fn test_delta_y() {
        assert_eq!(delta_y(&Point::new(MIN, 0, 0), &Point::new(MAX, 0, 0)), 0);
        assert_eq!(delta_y(&Point::new(0, MIN, 0), &Point::new(0, MAX, 0)), u64::MAX);
        assert_eq!(delta_y(&Point::new(0, 0, MIN), &Point::new(0, 0, MAX)), 0);
    }

    #[test]
    fn test_delta_z() {
        assert_eq!(delta_z(&Point::new(MIN, 0, 0), &Point::new(MAX, 0, 0)), 0);
        assert_eq!(delta_z(&Point::new(0, MIN, 0), &Point::new(0, MAX, 0)), 0);
        assert_eq!(delta_z(&Point::new(0, 0, MIN), &Point::new(0, 0, MAX)), u64::MAX);
    }
