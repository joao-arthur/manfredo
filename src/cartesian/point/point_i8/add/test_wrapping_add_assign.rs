...
mod tests {
    ...
    fn wrapping_add_to_bounds() {
        assert_eq!(wrapping_add(&PointI8::of(MIN + 2, MIN + 5), &PointI8::of(-2, -5)), PointI8::min());
        assert_eq!(wrapping_add(&PointI8::of(MAX - 2, MAX - 5), &PointI8::of(2, 5)), PointI8::max());
    }

    #[test]
    fn wrapping_add_out_of_bounds() {
        assert_eq!(wrapping_add(&PointI8::of(MIN + 2, MIN + 5), &PointI8::of(-10, -10)), PointI8::of(MAX - 7, MAX - 4));
        assert_eq!(wrapping_add(&PointI8::of(MAX - 2, MAX - 5), &PointI8::of(10, 10)), PointI8::of(MIN + 7, MIN + 4));
    }

    #[test]
    fn wrapping_add_limits_out_of_bounds() {
        assert_eq!(wrapping_add(&PointI8::of(MIN + 1, MIN + 1), &PointI8::min()), PointI8::of(1, 1));
        assert_eq!(wrapping_add(&PointI8::of(MAX - 1, MAX - 1), &PointI8::max()), PointI8::of(-3, -3));
    }
}
