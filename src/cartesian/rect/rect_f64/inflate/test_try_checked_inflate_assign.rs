...
#[test]
fn min_bounds() {
    let mut r = RectF64::of(MIN + 7.0, MIN + 3.0, MIN + 9.0, MIN + 13.0);
    assert_eq!(try_checked_inflate_assign(&mut r), Some(()));
    assert_eq!(r, RectF64::of(MIN + 6.0, MIN + 2.0, MIN + 10.0, MIN + 14.0));
    assert_eq!(try_checked_inflate_assign(&mut r), Some(()));
    assert_eq!(r, RectF64::of(MIN + 5.0, MIN + 1.0, MIN + 11.0, MIN + 15.0));
    assert_eq!(try_checked_inflate_assign(&mut r), Some(()));
    assert_eq!(r, RectF64::of(MIN + 4.0, MIN, MIN + 12.0, MIN + 16.0));
}
...
fn out_of_bounds() {
    ...
    let mut r_min_x = RectF64::of(MIN, MIN + 9.0, MAX - 9.0, MAX - 9.0);
    assert_eq!(try_checked_inflate_assign(&mut r_min_x), None);
    assert_eq!(r_min_x, RectF64::of(MIN, MIN + 9.0, MAX - 9.0, MAX - 9.0));

    let mut r_min_y = RectF64::of(MIN + 9.0, MIN, MAX - 9.0, MAX - 9.0);
    assert_eq!(try_checked_inflate_assign(&mut r_min_y), None);
    assert_eq!(r_min_y, RectF64::of(MIN + 9.0, MIN, MAX - 9.0, MAX - 9.0));

    let mut r_max_x = RectF64::of(MIN + 9.0, MIN + 9.0, MAX, MAX - 9.0);
    assert_eq!(try_checked_inflate_assign(&mut r_max_x), None);
    assert_eq!(r_max_x, RectF64::of(MIN + 9.0, MIN + 9.0, MAX, MAX - 9.0));

    let mut r_max_y = RectF64::of(MIN + 9.0, MIN + 9.0, MAX - 9.0, MAX);
