...
#[test]
fn out_of_bounds() {
    let mut r_min_x = RectF64::of(MIN, MIN + 2.0, MIN + 2.0, MIN + 4.0);
    assert_eq!(try_checked_resize_assign(&mut r_min_x, 5.0), None);
    assert_eq!(r_min_x, RectF64::of(MIN, MIN + 2.0, MIN + 2.0, MIN + 4.0));

    let mut r_min_y = RectF64::of(MIN + 2.0, MIN, MIN + 4.0, MIN + 2.0);
    assert_eq!(try_checked_resize_assign(&mut r_min_y, 5.0), None);
    assert_eq!(r_min_y, RectF64::of(MIN + 2.0, MIN, MIN + 4.0, MIN + 2.0));

    let mut r_max_x = RectF64::of(MAX - 2.0, MAX - 4.0, MAX, MAX - 2.0);
    assert_eq!(try_checked_resize_assign(&mut r_max_x, 5.0), None);
    assert_eq!(r_max_x, RectF64::of(MAX - 2.0, MAX - 4.0, MAX, MAX - 2.0));

    let mut r_max_y = RectF64::of(MAX - 4.0, MAX - 2.0, MAX - 2.0, MAX);
    assert_eq!(try_checked_resize_assign(&mut r_max_y, 5.0), None);
    assert_eq!(r_max_y, RectF64::of(MAX - 4.0, MAX - 2.0, MAX - 2.0, MAX));
}
