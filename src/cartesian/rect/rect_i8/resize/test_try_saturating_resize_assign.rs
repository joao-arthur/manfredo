...
#[test]
fn bounds() {
    let mut r_min = RectI8::of(MIN, MIN, MIN + 2, MIN + 2);
    assert_eq!(try_saturating_resize_assign(&mut r_min, 11), Some(()));
    assert_eq!(r_min, RectI8::of(MIN, MIN, MIN + 10, MIN + 10));

    let mut r_max = RectI8::of(MAX - 2, MAX - 2, MAX, MAX);
    assert_eq!(try_saturating_resize_assign(&mut r_max, 11), Some(()));
    assert_eq!(r_max, RectI8::of(MAX - 10, MAX - 10, MAX, MAX));
}
...
fn small_rect_limits() {
    ...
    assert_eq!(r_min, RectI8::of(MIN, MIN, MAX - 1, MAX - 1));

    let mut r_max = RectI8::of(MAX - 2, MAX - 2, MAX, MAX);
    assert_eq!(try_saturating_resize_assign(&mut r_max, u8::MAX), Some(()));
    assert_eq!(r_max, RectI8::of(MIN + 1, MIN + 1, MAX, MAX));
}
