use crate::cartesian::{
    ...
    rect::{rect_i32::Rect as RectI, rect_u32::RectU32},
};

pub fn try_checked_add_assign(r: &mut RectU32, delta: &RectI) -> Option<()> {
...
pub fn try_checked_add(r: &RectU32, delta: &RectI) -> Option<RectU32> {
    let min_x = r.min.x.checked_add_signed(delta.min.x)?;
    let min_y = r.min.y.checked_add_signed(delta.min.y)?;
    let max_x = r.max.x.checked_add_signed(delta.max.x)?;
    let max_y = r.max.y.checked_add_signed(delta.max.y)?;
...
pub fn checked_add_assign(r: &mut RectU32, delta: &RectI) {
    try_checked_add_assign(r, delta).unwrap()
}

pub fn checked_add(r: &RectU32, delta: &RectI) -> RectU32 {
    try_checked_add(r, delta).unwrap()
}

pub fn saturating_add_assign(r: &mut RectU32, delta: &RectI) {
...
pub fn saturating_add(r: &RectU32, delta: &RectI) -> RectU32 {
...
pub fn wrapping_add_assign(r: &mut RectU32, delta: &RectI) {
...
pub fn wrapping_add(r: &RectU32, delta: &RectI) -> RectU32 {
