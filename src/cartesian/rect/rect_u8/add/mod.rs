use crate::cartesian::{
    ...
    rect::{rect_i8::Rect as RectI, rect_u8::RectU8},
};

pub fn try_checked_add_assign(r: &mut RectU8, delta: &RectI) -> Option<()> {
...
pub fn try_checked_add(r: &RectU8, delta: &RectI) -> Option<RectU8> {
    let min_x = r.min.x.checked_add_signed(delta.min.x)?;
    let min_y = r.min.y.checked_add_signed(delta.min.y)?;
    let max_x = r.max.x.checked_add_signed(delta.max.x)?;
    let max_y = r.max.y.checked_add_signed(delta.max.y)?;
...
pub fn checked_add_assign(r: &mut RectU8, delta: &RectI) {
    try_checked_add_assign(r, delta).unwrap()
}

pub fn checked_add(r: &RectU8, delta: &RectI) -> RectU8 {
    try_checked_add(r, delta).unwrap()
}

pub fn saturating_add_assign(r: &mut RectU8, delta: &RectI) {
...
pub fn saturating_add(r: &RectU8, delta: &RectI) -> RectU8 {
...
pub fn wrapping_add_assign(r: &mut RectU8, delta: &RectI) {
...
pub fn wrapping_add(r: &RectU8, delta: &RectI) -> RectU8 {
