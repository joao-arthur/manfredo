...
pub fn assign_inflate_in_bounds(r: &mut RectU8) -> Option<()> {
...
pub fn inflate_in_bounds(r: &RectU8) -> Option<RectU8> {
    let is_min_x = r.min.x == 0;
    let is_min_y = r.min.y == 0;
    let is_max_x = r.max.x == u8::MAX;
    let is_max_y = r.max.y == u8::MAX;
...
#[cfg(test)]
mod tests {
    use super::{RectU8, assign_inflate_in_bounds,  inflate_in_bounds};

    #[test]
    fn assign_inflate_in_bounds_min_bounds() {
        let mut r = RectU8::of(7, 2, 4, 13);
        assert_eq!(assign_inflate_in_bounds(&mut r), Some(()));
