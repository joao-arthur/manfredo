use super::try_checked_translate_assign;
use crate::cartesian::{point::point_i8::Point, rect::rect_u8::Rect};

const MAX: u8 = u8::MAX;

#[test]
fn test() {
    let mut r = Rect::of(0, 0, 12, 15);
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(5, 4)), Some(()));
    assert_eq!(r, Rect::of(5, 4, 17, 19));
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(-4, -2)), Some(()));
    assert_eq!(r, Rect::of(1, 2, 13, 17));
}

#[test]
fn to_bounds() {
    let mut r_min = Rect::of(2, 5, MAX, MAX);
    assert_eq!(try_checked_translate_assign(&mut r_min, &Point::of(-2, -5)), Some(()));
    assert_eq!(r_min, Rect::of(0, 0, MAX - 2, MAX - 5));

    let mut r_max = Rect::of(0, 0, MAX - 2, MAX - 5);
    assert_eq!(try_checked_translate_assign(&mut r_max, &Point::of(2, 5)), Some(()));
    assert_eq!(r_max, Rect::of(2, 5, MAX, MAX));
}

#[test]
fn out_of_bounds() {
    let mut r = Rect::of(10, 10, MAX - 10, MAX - 10);
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(-20, 0)), None);
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(0, -20)), None);
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(20, 0)), None);
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(0, 20)), None);
    assert_eq!(r, Rect::of(10, 10, MAX - 10, MAX - 10));
}

#[test]
fn limits_out_of_bounds() {
    let mut r = Rect::largest();
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(i8::MIN, 0)), None);
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(0, i8::MIN)), None);
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(i8::MAX, 0)), None);
    assert_eq!(try_checked_translate_assign(&mut r, &Point::of(0, i8::MAX)), None);
    assert_eq!(r, Rect::largest());
}
