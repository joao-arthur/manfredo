#[cfg(test)]
mod tests {
    use super::super::{wrapping_translate, wrapping_translate_assign};
    use crate::cartesian::{point::point_i8::PointI8, rect::rect_u8::RectU8};

    #[test]
    fn test_wrapping_translate_assign() {
        let mut r = RectU8::of(0, 0, 12, 15);
        wrapping_translate_assign(&mut r, &PointI8::of(5, 4));
        assert_eq!(r, RectU8::of(5, 4, 17, 19));
        wrapping_translate_assign(&mut r, &PointI8::of(-4, -2));
        assert_eq!(r, RectU8::of(1, 2, 13, 17));
    }

    #[test]
    fn wrapping_translate_assign_small_rect_to_bounds() {
        let mut min_r = RectU8::of(2, 5, 12, 15);
        wrapping_translate_assign(&mut min_r, &PointI8::of(-2, -5));
        assert_eq!(min_r, RectU8::of(0, 0, 10, 10));

        let mut max_r = RectU8::of(u8::MAX - 12, u8::MAX - 15, u8::MAX - 2, u8::MAX - 5);
        wrapping_translate_assign(&mut max_r, &PointI8::of(2, 5));
        assert_eq!(max_r, RectU8::of(u8::MAX - 10, u8::MAX - 10, u8::MAX, u8::MAX));
    }

    #[test]
    fn wrapping_translate_assign_big_rect_to_bounds() {
        let mut min_r = RectU8::of(2, 5, u8::MAX, u8::MAX);
        wrapping_translate_assign(&mut min_r, &PointI8::of(-2, -5));
        assert_eq!(min_r, RectU8::of(0, 0, u8::MAX - 2, u8::MAX - 5));

        let mut max_r = RectU8::of(0, 0, u8::MAX - 2, u8::MAX - 5);
        wrapping_translate_assign(&mut max_r, &PointI8::of(2, 5));
        assert_eq!(max_r, RectU8::of(2, 5, u8::MAX, u8::MAX));
    }

    #[test]
    fn wrapping_translate_assign_small_rect_out_of_bounds() {
        let mut r_min = RectU8::of(10, 5, 20, 30);
        wrapping_translate_assign(&mut r_min, &PointI8::of(-20, -20));
        assert_eq!(r_min, RectU8::of(u8::MAX - 9, u8::MAX - 14, 0, 10));

        let mut r_max = RectU8::of(u8::MAX - 20, u8::MAX - 30, u8::MAX - 5, u8::MAX - 10);
        wrapping_translate_assign(&mut r_max, &PointI8::of(20, 20));
        assert_eq!(r_max, RectU8::of(u8::MAX, u8::MAX - 10, 14, 9));
    }

    #[test]
    fn wrapping_translate_assign_big_rect_out_of_bounds() {
        let mut r_min = RectU8::of(10, 5, u8::MAX, u8::MAX);
        wrapping_translate_assign(&mut r_min, &PointI8::of(-20, -20));
        assert_eq!(r_min, RectU8::of(u8::MAX - 9, u8::MAX - 14, u8::MAX - 20, u8::MAX - 20));

        let mut r_max = RectU8::of(0, 0, u8::MAX - 5, u8::MAX - 10);
        wrapping_translate_assign(&mut r_max, &PointI8::of(20, 20));
        assert_eq!(r_max, RectU8::of(20, 20, 14, 9));
    }

    #[test]
    fn wrapping_translate_assign_small_rect_limits_out_of_bounds() {
        let mut r_min = RectU8::of(1, 1, 10, 10);
        wrapping_translate_assign(&mut r_min, &PointI8::min());
        assert_eq!(r_min, RectU8::of((i8::MAX as u8) + 2, (i8::MAX as u8) + 2, (i8::MAX as u8) + 11, (i8::MAX as u8) + 11));

        let mut r_max = RectU8::of(u8::MAX - 10, u8::MAX - 10, u8::MAX - 1, u8::MAX - 1);
        wrapping_translate_assign(&mut r_max, &PointI8::max());
        assert_eq!(r_max, RectU8::of((i8::MAX as u8) - 11, (i8::MAX as u8) - 11, (i8::MAX as u8) - 2, (i8::MAX as u8) - 2));
    }

    #[test]
    fn wrapping_translate_assign_big_rect_limits_out_of_bounds() {
        let mut r1 = RectU8::largest();
        wrapping_translate_assign(&mut r1, &PointI8::min());
        assert_eq!(r1, RectU8::of((i8::MAX as u8) + 1, (i8::MAX as u8) + 1, i8::MAX as u8, i8::MAX as u8));

        let mut r2 = RectU8::largest();
        wrapping_translate_assign(&mut r2, &PointI8::max());
        assert_eq!(r2, RectU8::of(i8::MAX as u8, i8::MAX as u8, (i8::MAX as u8) - 1, (i8::MAX as u8) - 1));

        let mut r_min = RectU8::of(1, 1, u8::MAX, u8::MAX);
        wrapping_translate_assign(&mut r_min, &PointI8::min());
        assert_eq!(r_min, RectU8::of((i8::MAX as u8) + 2, (i8::MAX as u8) + 2, i8::MAX as u8, i8::MAX as u8));

        let mut r_max = RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1);
        wrapping_translate_assign(&mut r_max, &PointI8::max());
        assert_eq!(r_max, RectU8::of(i8::MAX as u8, i8::MAX as u8, (i8::MAX as u8) - 2, (i8::MAX as u8) - 2));
    }

    #[test]
    fn test_wrapping_translate() {
        assert_eq!(wrapping_translate(&RectU8::of(0, 0, 12, 15), &PointI8::of(5, 4)), RectU8::of(5, 4, 17, 19));
        assert_eq!(wrapping_translate(&RectU8::of(5, 4, 17, 19), &PointI8::of(-4, -2)), RectU8::of(1, 2, 13, 17));
    }

    #[test]
    fn wrapping_translate_small_rect_to_bounds() {
        assert_eq!(wrapping_translate(&RectU8::of(2, 5, 12, 15), &PointI8::of(-2, -5)), RectU8::of(0, 0, 10, 10));
        assert_eq!(
            wrapping_translate(&RectU8::of(u8::MAX - 12, u8::MAX - 15, u8::MAX - 2, u8::MAX - 5), &PointI8::of(2, 5)),
            RectU8::of(u8::MAX - 10, u8::MAX - 10, u8::MAX, u8::MAX)
        );
    }

    #[test]
    fn wrapping_translate_big_rect_to_bounds() {
        assert_eq!(wrapping_translate(&RectU8::of(2, 5, u8::MAX, u8::MAX), &PointI8::of(-2, -5)), RectU8::of(0, 0, u8::MAX - 2, u8::MAX - 5));
        assert_eq!(wrapping_translate(&RectU8::of(0, 0, u8::MAX - 2, u8::MAX - 5), &PointI8::of(2, 5)), RectU8::of(2, 5, u8::MAX, u8::MAX));
    }

    #[test]
    fn wrapping_translate_small_rect_out_of_bounds() {
        assert_eq!(wrapping_translate(&RectU8::of(10, 5, 20, 30), &PointI8::of(-20, -20)), RectU8::of(u8::MAX - 9, u8::MAX - 14, 0, 10));
        assert_eq!(
            wrapping_translate(&RectU8::of(u8::MAX - 20, u8::MAX - 30, u8::MAX - 5, u8::MAX - 10), &PointI8::of(20, 20)),
            RectU8::of(u8::MAX, u8::MAX - 10, 14, 9)
        );
    }

    #[test]
    fn wrapping_translate_big_rect_out_of_bounds() {
        assert_eq!(
            wrapping_translate(&RectU8::of(10, 5, u8::MAX, u8::MAX), &PointI8::of(-20, -20)),
            RectU8::of(u8::MAX - 9, u8::MAX - 14, u8::MAX - 20, u8::MAX - 20)
        );
        assert_eq!(wrapping_translate(&RectU8::of(0, 0, u8::MAX - 5, u8::MAX - 10), &PointI8::of(20, 20)), RectU8::of(20, 20, 14, 9));
    }

    #[test]
    fn wrapping_translate_small_rect_limits_out_of_bounds() {
        assert_eq!(
            wrapping_translate(&RectU8::of(1, 1, 10, 10), &PointI8::min()),
            RectU8::of((i8::MAX as u8) + 2, (i8::MAX as u8) + 2, (i8::MAX as u8) + 11, (i8::MAX as u8) + 11)
        );
        assert_eq!(
            wrapping_translate(&RectU8::of(u8::MAX - 10, u8::MAX - 10, u8::MAX - 1, u8::MAX - 1), &PointI8::max()),
            RectU8::of((i8::MAX as u8) - 11, (i8::MAX as u8) - 11, (i8::MAX as u8) - 2, (i8::MAX as u8) - 2)
        );
    }

    #[test]
    fn wrapping_translate_big_rect_limits_out_of_bounds() {
        assert_eq!(
            wrapping_translate(&RectU8::largest(), &PointI8::min()),
            RectU8::of((i8::MAX as u8) + 1, (i8::MAX as u8) + 1, i8::MAX as u8, i8::MAX as u8)
        );
        assert_eq!(
            wrapping_translate(&RectU8::largest(), &PointI8::max()),
            RectU8::of(i8::MAX as u8, i8::MAX as u8, (i8::MAX as u8) - 1, (i8::MAX as u8) - 1)
        );
        assert_eq!(
            wrapping_translate(&RectU8::of(1, 1, u8::MAX, u8::MAX), &PointI8::min()),
            RectU8::of((i8::MAX as u8) + 2, (i8::MAX as u8) + 2, i8::MAX as u8, i8::MAX as u8)
        );
        assert_eq!(
            wrapping_translate(&RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1), &PointI8::max()),
            RectU8::of(i8::MAX as u8, i8::MAX as u8, (i8::MAX as u8) - 2, (i8::MAX as u8) - 2)
        );
    }
}
