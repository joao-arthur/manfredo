...
mod tests {
    ...
    use crate::matrix::{d1::point::point_u64::MAX, d2::point::point_u64::Point};

    #[test]
    fn test_delta_row() {
        assert_eq!(delta_row(&Point::min(), &Point::new(MAX, 0)), MAX);
        assert_eq!(delta_row(&Point::min(), &Point::new(0, MAX)), 0);
    }

    #[test]
    fn test_delta_col() {
        assert_eq!(delta_col(&Point::min(), &Point::new(MAX, 0)), 0);
        assert_eq!(delta_col(&Point::min(), &Point::new(0, MAX)), MAX);
    }
