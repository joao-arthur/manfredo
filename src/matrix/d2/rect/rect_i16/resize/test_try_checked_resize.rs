...
fn small_rect_limits_out_of_bounds() {
    assert_eq!(try_checked_resize(&Rect::of(MIN, MIN + 2, MIN + 2, MIN + 4), u16::MAX), None);
    assert_eq!(try_checked_resize(&Rect::of(MIN + 2, MIN, MIN + 4, MIN + 2), u16::MAX), None);
    assert_eq!(try_checked_resize(&Rect::of(MAX - 2, MAX - 4, MAX, MAX - 2), u16::MAX), None);
    assert_eq!(try_checked_resize(&Rect::of(MAX - 4, MAX - 2, MAX - 2, MAX), u16::MAX), None);
}

#[test]
fn big_rect_limits_out_of_bounds() {
    assert_eq!(try_checked_resize(&Rect::of(MIN, MIN, MAX - 1, MAX - 1), u16::MAX), Some(Rect::of(MIN, MIN, MAX - 1, MAX - 1)));
    assert_eq!(try_checked_resize(&Rect::of(MIN + 1, MIN + 1, MAX, MAX), u16::MAX), Some(Rect::of(MIN + 1, MIN + 1, MAX, MAX)));
    assert_eq!(try_checked_resize(&Rect::largest(), u16::MAX), Some(Rect::of(MIN, MIN, MAX - 1, MAX - 1)));
}
