...
fn small_rect_limits_out_of_bounds() {
    assert_eq!(try_wrapping_resize(&Rect::of(MIN, MIN + 2, MIN + 2, MIN + 4), u8::MAX), Some(Rect::of(2, 4, 0, 2)));
    assert_eq!(try_wrapping_resize(&Rect::of(MIN + 2, MIN, MIN + 4, MIN + 2), u8::MAX), Some(Rect::of(4, 2, 2, 0)));
    assert_eq!(try_wrapping_resize(&Rect::of(MAX - 2, MAX - 4, MAX, MAX - 2), u8::MAX), Some(Rect::of(-1, -3, -3, -5)));
    assert_eq!(try_wrapping_resize(&Rect::of(MAX - 4, MAX - 2, MAX - 2, MAX), u8::MAX), Some(Rect::of(-3, -1, -5, -3)));
}

#[test]
fn big_rect_limits_out_of_bounds() {
    assert_eq!(try_wrapping_resize(&Rect::of(MIN, MIN, MAX - 1, MAX - 1), u8::MAX), Some(Rect::of(MIN, MIN, MAX - 1, MAX - 1)));
    assert_eq!(try_wrapping_resize(&Rect::of(MIN + 1, MIN + 1, MAX, MAX), u8::MAX), Some(Rect::of(MIN + 1, MIN + 1, MAX, MAX)));
    assert_eq!(try_wrapping_resize(&Rect::largest(), u8::MAX), Some(Rect::of(MIN, MIN, MAX - 1, MAX - 1)));
}
