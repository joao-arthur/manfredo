...
fn limits_out_of_bounds() {
    ...
    wrapping_translate_assign(&mut r1, &Point::of(i32::MIN, 0));
    assert_eq!(r1, Rect::of(MAX / 2 + 1, 0, MAX / 2, MAX));

    let mut r2 = Rect::largest();
    wrapping_translate_assign(&mut r2, &Point::of(0, i32::MIN));
    assert_eq!(r2, Rect::of(0, MAX / 2 + 1, MAX, MAX / 2));

    let mut r3 = Rect::largest();
    wrapping_translate_assign(&mut r3, &Point::of(i32::MAX, 0));
    assert_eq!(r3, Rect::of(MAX / 2, 0, MAX / 2 - 1, MAX));

    let mut r4 = Rect::largest();
    wrapping_translate_assign(&mut r4, &Point::of(0, i32::MAX));
