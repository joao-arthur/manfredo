...
#[test]
fn out_of_bounds() {
    let mut r_min_row = RectI64::of(MIN, MIN + 2, MIN + 2, MIN + 4);
    assert_eq!(try_wrapping_resize_assign(&mut r_min_row, 5), Some(()));
    assert_eq!(r_min_row, RectI64::of(MAX, MIN + 1, MIN + 3, MIN + 5));

    let mut r_min_y = RectI64::of(MIN + 2, MIN, MIN + 4, MIN + 2);
    assert_eq!(try_wrapping_resize_assign(&mut r_min_y, 5), Some(()));
    assert_eq!(r_min_y, RectI64::of(MIN + 1, MAX, MIN + 5, MIN + 3));

    let mut r_max_x = RectI64::of(MAX - 2, MAX - 4, MAX, MAX - 2);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_x, 5), Some(()));
    assert_eq!(r_max_x, RectI64::of(MAX - 3, MAX - 5, MIN, MAX - 1));

    let mut r_max_y = RectI64::of(MAX - 4, MAX - 2, MAX - 2, MAX);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_y, 5), Some(()));
    assert_eq!(r_max_y, RectI64::of(MAX - 5, MAX - 3, MAX - 1, MIN));
}
...
fn small_rect_limits_out_of_bounds() {
    ...
    assert_eq!(r_min_row, RectI64::of(2, 4, 0, 2));
