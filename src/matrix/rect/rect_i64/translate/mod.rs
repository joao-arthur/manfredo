use crate::matrix::{
    ...
    rect::rect_i64::{Rect, delta_col, delta_row},
};

pub fn try_checked_translate_assign(r: &mut Rect, delta: &PointI64) -> Option<()> {
...
pub fn try_checked_translate(r: &Rect, delta: &PointI64) -> Option<Rect> {
    let min_row = r.min.row.checked_add(delta.row)?;
    let min_col = r.min.col.checked_add(delta.col)?;
    let max_row = r.max.row.checked_add(delta.row)?;
    let max_col = r.max.col.checked_add(delta.col)?;
    Some(Rect { min: PointI64 { row: min_row, col: min_col }, max: PointI64 { row: max_row, col: max_col } })
}
...
pub fn checked_translate(r: &Rect, delta: &PointI64) -> Rect {
    try_checked_translate(r, delta).unwrap()
}

pub fn saturating_translate_assign(r: &mut Rect, delta: &PointI64) {
...
pub fn saturating_translate(r: &Rect, delta: &PointI64) -> Rect {
    let d_row = delta_row(r);
    let d_col = delta_col(r);
    let temp_min_row = i128::from(r.min.row) + i128::from(delta.row);
    let temp_min_col = i128::from(r.min.col) + i128::from(delta.col);
    ...
    Rect { min: PointI64 { row: min_row as i64, col: min_col as i64 }, max: PointI64 { row: max_row as i64, col: max_col as i64 } }
}
...
pub fn wrapping_translate(r: &Rect, delta: &PointI64) -> Rect {
    let d_row = delta_row(r);
    let d_col = delta_col(r);
    let min_row = r.min.row.wrapping_add(delta.row);
    let min_col = r.min.col.wrapping_add(delta.col);
    let max_row = min_row.wrapping_add_unsigned(d_row);
    let max_col = min_col.wrapping_add_unsigned(d_col);
    Rect { min: PointI64 { row: min_row, col: min_col }, max: PointI64 { row: max_row, col: max_col } }
}
