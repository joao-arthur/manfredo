...
fn to_bounds() {
    ...
    let mut r_min = RectI8::of(MIN, MIN, MAX - 1, MAX - 1);
    assert_eq!(try_saturating_inflate_assign(&mut r_min), Some(()));
    assert_eq!(r_min, RectI8::largest());

    let mut r_max = RectI8::of(MIN + 1, MIN + 1, MAX, MAX);
...
fn out_of_bounds() {
    ...
    let mut r_x = RectI8::of(MIN, MIN + 10, MAX, MAX - 10);
    assert_eq!(try_saturating_inflate_assign(&mut r_x), None);
    assert_eq!(r_x, RectI8::of(MIN, MIN + 10, MAX, MAX - 10));

    let mut r_y = RectI8::of(MIN + 10, MIN, MAX - 10, MAX);
    assert_eq!(try_saturating_inflate_assign(&mut r_y), None);
    assert_eq!(r_y, RectI8::of(MIN + 10, MIN, MAX - 10, MAX));
}
