...
fn out_of_bounds() {
    ...
    let mut r_min_row = RectU16::of(0, 9, MAX - 9, MAX - 9);
    assert_eq!(try_checked_inflate_assign(&mut r_min_row), None);
    assert_eq!(r_min_row, RectU16::of(0, 9, MAX - 9, MAX - 9));

    let mut r_min_y = RectU16::of(9, 0, MAX - 9, MAX - 9);
    assert_eq!(try_checked_inflate_assign(&mut r_min_y), None);
    assert_eq!(r_min_y, RectU16::of(9, 0, MAX - 9, MAX - 9));

    let mut r_max_x = RectU16::of(9, 9, MAX, MAX - 9);
    assert_eq!(try_checked_inflate_assign(&mut r_max_x), None);
    assert_eq!(r_max_x, RectU16::of(9, 9, MAX, MAX - 9));

    let mut r_max_y = RectU16::of(9, 9, MAX - 9, MAX);
