...
fn same_size() {
    ...
    let mut r_max_2 = RectU64::of(MAX - 2, MAX - 2, MAX, MAX);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_2, 3), Some(()));
    assert_eq!(r_max_2, RectU64::of(MAX - 2, MAX - 2, MAX, MAX));

    let mut r_max_3 = RectU64::of(MAX - 3, MAX - 3, MAX, MAX);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_3, 4), Some(()));
    assert_eq!(r_max_3, RectU64::of(MAX - 3, MAX - 3, MAX, MAX));
}
...
fn out_of_bounds() {
    ...
    assert_eq!(r_min_row, RectU64::of(MAX, 1, 3, 5));

    let mut r_min_y = RectU64::of(2, 0, 4, 2);
    assert_eq!(try_wrapping_resize_assign(&mut r_min_y, 5), Some(()));
    assert_eq!(r_min_y, RectU64::of(1, MAX, 5, 3));

    let mut r_max_x = RectU64::of(MAX - 2, MAX - 4, MAX, MAX - 2);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_x, 5), Some(()));
    assert_eq!(r_max_x, RectU64::of(MAX - 3, MAX - 5, 0, MAX - 1));

    let mut r_max_y = RectU64::of(MAX - 4, MAX - 2, MAX - 2, MAX);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_y, 5), Some(()));
    assert_eq!(r_max_y, RectU64::of(MAX - 5, MAX - 3, MAX - 1, 0));
}
...
fn small_rect_limits_out_of_bounds() {
    ...
    assert_eq!(r_min_row, RectU64::of(MAX / 2 + 3, MAX / 2 + 5, MAX / 2 + 1, MAX / 2 + 3));

    let mut r_min_y = RectU64::of(2, 0, 4, 2);
    assert_eq!(try_wrapping_resize_assign(&mut r_min_y, MAX), Some(()));
    assert_eq!(r_min_y, RectU64::of(MAX / 2 + 5, MAX / 2 + 3, MAX / 2 + 3, MAX / 2 + 1));

    let mut r_max_x = RectU64::of(MAX - 2, MAX - 4, MAX, MAX - 2);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_x, MAX), Some(()));
    assert_eq!(r_max_x, RectU64::of(MAX / 2, MAX / 2 - 2, MAX / 2 - 2, MAX / 2 - 4));

    let mut r_max_y = RectU64::of(MAX - 4, MAX - 2, MAX - 2, MAX);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_y, MAX), Some(()));
    assert_eq!(r_max_y, RectU64::of(MAX / 2 - 2, MAX / 2, MAX / 2 - 4, MAX / 2 - 2));
}

#[test]
fn big_rect_limits_out_of_bounds() {
    let mut r_odd_1 = RectU64::of(0, 0, MAX - 1, MAX - 1);
    assert_eq!(try_wrapping_resize_assign(&mut r_odd_1, MAX), Some(()));
    assert_eq!(r_odd_1, RectU64::of(0, 0, MAX - 1, MAX - 1));

    let mut r_odd_1 = RectU64::of(1, 1, MAX, MAX);
    assert_eq!(try_wrapping_resize_assign(&mut r_odd_1, MAX), Some(()));
    assert_eq!(r_odd_1, RectU64::of(1, 1, MAX, MAX));

    let mut r_even = RectU64::largest();
    assert_eq!(try_wrapping_resize_assign(&mut r_even, MAX), Some(()));
    assert_eq!(r_even, RectU64::of(0, 0, MAX - 1, MAX - 1));
}
