...
pub fn try_wrapping_resize_assign(r: &mut RectU8, size: u8) -> Option<()> {
    if size < 3 {
        return None;
    }
    let diff_row = i16::from(delta_row(r)) + 1 - i16::from(size);
    ...
    let temp_min_row = i16::from(r.min.row) + diff_row / 2;
    let temp_min_col = i16::from(r.min.col) + diff_col / 2;
    let min_row = temp_min_row as u8;
    let min_col = temp_min_col as u8;
...
pub fn try_wrapping_resize(r: &RectU8, size: u8) -> Option<RectU8> {
    if size < 3 {
        return None;
    }
    let diff_row = i16::from(delta_row(r)) + 1 - i16::from(size);
...
pub fn wrapping_resize_assign(r: &mut RectU8, size: u8) {
    try_wrapping_resize_assign(r, size).unwrap()
}

pub fn wrapping_resize(r: &RectU8, size: u8) -> RectU8 {
