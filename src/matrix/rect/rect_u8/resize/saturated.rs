#[cfg(test)]
mod tests {
    use super::super::{saturating_resize, saturating_resize_assign, try_saturating_resize, try_saturating_resize_assign};
    use crate::matrix::rect::rect_u8::RectU8;

    #[test]
    fn try_saturating_resize_assign_odd() {
        let mut r = RectU8::of(5, 5, 15, 15);
        assert_eq!(try_saturating_resize_assign(&mut r, 11), Some(()));
        assert_eq!(r, RectU8::of(5, 5, 15, 15));
        assert_eq!(try_saturating_resize_assign(&mut r, 9), Some(()));
        assert_eq!(r, RectU8::of(6, 6, 14, 14));
        assert_eq!(try_saturating_resize_assign(&mut r, 7), Some(()));
        assert_eq!(r, RectU8::of(7, 7, 13, 13));
        assert_eq!(try_saturating_resize_assign(&mut r, 5), Some(()));
        assert_eq!(r, RectU8::of(8, 8, 12, 12));
        assert_eq!(try_saturating_resize_assign(&mut r, 3), Some(()));
        assert_eq!(r, RectU8::of(9, 9, 11, 11));
        assert_eq!(try_saturating_resize_assign(&mut r, 9), Some(()));
        assert_eq!(r, RectU8::of(6, 6, 14, 14));
    }

    #[test]
    fn try_saturating_resize_assign_even() {
        let mut r = RectU8::of(5, 5, 14, 14);
        assert_eq!(try_saturating_resize_assign(&mut r, 10), Some(()));
        assert_eq!(r, RectU8::of(5, 5, 14, 14));
        assert_eq!(try_saturating_resize_assign(&mut r, 8), Some(()));
        assert_eq!(r, RectU8::of(6, 6, 13, 13));
        assert_eq!(try_saturating_resize_assign(&mut r, 6), Some(()));
        assert_eq!(r, RectU8::of(7, 7, 12, 12));
        assert_eq!(try_saturating_resize_assign(&mut r, 4), Some(()));
        assert_eq!(r, RectU8::of(8, 8, 11, 11));
        assert_eq!(try_saturating_resize_assign(&mut r, 8), Some(()));
        assert_eq!(r, RectU8::of(6, 6, 13, 13));
    }

    #[test]
    fn try_saturating_resize_assign_small_size() {
        let mut r = RectU8::of(10, 10, 20, 20);
        assert_eq!(try_saturating_resize_assign(&mut r, 0), None);
        assert_eq!(try_saturating_resize_assign(&mut r, 1), None);
        assert_eq!(try_saturating_resize_assign(&mut r, 2), None);
        assert_eq!(r, RectU8::of(10, 10, 20, 20));
    }

    #[test]
    fn try_saturating_resize_assign_same_size() {
        let mut r_11 = RectU8::of(10, 10, 20, 20);
        assert_eq!(try_saturating_resize_assign(&mut r_11, 11), Some(()));
        assert_eq!(r_11, RectU8::of(10, 10, 20, 20));

        let mut r_12 = RectU8::of(10, 10, 21, 21);
        assert_eq!(try_saturating_resize_assign(&mut r_12, 12), Some(()));
        assert_eq!(r_12, RectU8::of(10, 10, 21, 21));

        let mut r_13 = RectU8::of(9, 9, 21, 21);
        assert_eq!(try_saturating_resize_assign(&mut r_13, 13), Some(()));
        assert_eq!(r_13, RectU8::of(9, 9, 21, 21));
    }

    #[test]
    fn try_saturating_resize_assign_odd_small_rect_out_of_bounds() {
        let mut r_min = RectU8::of(0, 0, 2, 2);
        assert_eq!(try_saturating_resize_assign(&mut r_min, 11), Some(()));
        assert_eq!(r_min, RectU8::of(0, 0, 10, 10));

        let mut r_max = RectU8::of(u8::MAX - 2, u8::MAX - 2, u8::MAX, u8::MAX);
        assert_eq!(try_saturating_resize_assign(&mut r_max, 11), Some(()));
        assert_eq!(r_max, RectU8::of(u8::MAX - 10, u8::MAX - 10, u8::MAX, u8::MAX));
    }

    #[test]
    fn try_saturating_resize_assign_even_small_rect_out_of_bounds() {
        let mut r = RectU8::of(0, 0, 3, 3);
        assert_eq!(try_saturating_resize_assign(&mut r, 11), Some(()));
        assert_eq!(r, RectU8::of(0, 0, 10, 10));

        let mut r = RectU8::of(u8::MAX - 3, u8::MAX - 3, u8::MAX, u8::MAX);
        assert_eq!(try_saturating_resize_assign(&mut r, 11), Some(()));
        assert_eq!(r, RectU8::of(u8::MAX - 10, u8::MAX - 10, u8::MAX, u8::MAX));
    }

    #[test]
    fn try_saturating_resize_assign_odd_small_rect_limits_out_of_bounds() {
        let mut r_min = RectU8::of(0, 0, 2, 2);
        assert_eq!(try_saturating_resize_assign(&mut r_min, u8::MAX), Some(()));
        assert_eq!(r_min, RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1));

        let mut r_max = RectU8::of(u8::MAX - 2, u8::MAX - 2, u8::MAX, u8::MAX);
        assert_eq!(try_saturating_resize_assign(&mut r_max, u8::MAX), Some(()));
        assert_eq!(r_max, RectU8::of(1, 1, u8::MAX, u8::MAX));
    }

    #[test]
    fn try_saturating_resize_assign_even_small_rect_limits_out_of_bounds() {
        let mut r = RectU8::of(0, 0, 3, 3);
        assert_eq!(try_saturating_resize_assign(&mut r, u8::MAX - 1), Some(()));
        assert_eq!(r, RectU8::of(0, 0, u8::MAX - 2, u8::MAX - 2));

        let mut r = RectU8::of(u8::MAX - 3, u8::MAX - 3, u8::MAX, u8::MAX);
        assert_eq!(try_saturating_resize_assign(&mut r, u8::MAX - 1), Some(()));
        assert_eq!(r, RectU8::of(2, 2, u8::MAX, u8::MAX));
    }

    #[test]
    fn try_saturating_resize_assign_big_rect_limits_out_of_bounds() {
        let mut r_odd_1 = RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1);
        assert_eq!(try_saturating_resize_assign(&mut r_odd_1, u8::MAX), Some(()));
        assert_eq!(r_odd_1, RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1));

        let mut r_odd_1 = RectU8::of(1, 1, u8::MAX, u8::MAX);
        assert_eq!(try_saturating_resize_assign(&mut r_odd_1, u8::MAX), Some(()));
        assert_eq!(r_odd_1, RectU8::of(1, 1, u8::MAX, u8::MAX));

        let mut r_even = RectU8::largest();
        assert_eq!(try_saturating_resize_assign(&mut r_even, u8::MAX), Some(()));
        assert_eq!(r_even, RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1));
    }

    #[test]
    fn try_saturating_resize_odd() {
        assert_eq!(try_saturating_resize(&RectU8::of(5, 5, 15, 15), 11), Some(RectU8::of(5, 5, 15, 15)));
        assert_eq!(try_saturating_resize(&RectU8::of(5, 5, 15, 15), 9), Some(RectU8::of(6, 6, 14, 14)));
        assert_eq!(try_saturating_resize(&RectU8::of(6, 6, 14, 14), 7), Some(RectU8::of(7, 7, 13, 13)));
        assert_eq!(try_saturating_resize(&RectU8::of(7, 7, 13, 13), 5), Some(RectU8::of(8, 8, 12, 12)));
        assert_eq!(try_saturating_resize(&RectU8::of(8, 8, 12, 12), 3), Some(RectU8::of(9, 9, 11, 11)));
        assert_eq!(try_saturating_resize(&RectU8::of(9, 9, 11, 11), 9), Some(RectU8::of(6, 6, 14, 14)));
    }

    #[test]
    fn try_saturating_resize_even() {
        assert_eq!(try_saturating_resize(&RectU8::of(5, 5, 14, 14), 10), Some(RectU8::of(5, 5, 14, 14)));
        assert_eq!(try_saturating_resize(&RectU8::of(5, 5, 14, 14), 8), Some(RectU8::of(6, 6, 13, 13)));
        assert_eq!(try_saturating_resize(&RectU8::of(6, 6, 13, 13), 6), Some(RectU8::of(7, 7, 12, 12)));
        assert_eq!(try_saturating_resize(&RectU8::of(7, 7, 12, 12), 4), Some(RectU8::of(8, 8, 11, 11)));
        assert_eq!(try_saturating_resize(&RectU8::of(8, 8, 11, 11), 8), Some(RectU8::of(6, 6, 13, 13)));
    }

    #[test]
    fn try_saturating_resize_small_size() {
        let r = RectU8::of(10, 10, 20, 20);
        assert_eq!(try_saturating_resize(&r, 0), None);
        assert_eq!(try_saturating_resize(&r, 1), None);
        assert_eq!(try_saturating_resize(&r, 2), None);
    }

    #[test]
    fn try_saturating_resize_same_size() {
        assert_eq!(try_saturating_resize(&RectU8::of(10, 10, 20, 20), 11), Some(RectU8::of(10, 10, 20, 20)));
        assert_eq!(try_saturating_resize(&RectU8::of(10, 10, 21, 21), 12), Some(RectU8::of(10, 10, 21, 21)));
        assert_eq!(try_saturating_resize(&RectU8::of(9, 9, 21, 21), 13), Some(RectU8::of(9, 9, 21, 21)));
    }

    #[test]
    fn try_saturating_resize_odd_small_rect_out_of_bounds() {
        assert_eq!(try_saturating_resize(&RectU8::of(0, 0, 2, 2), 11), Some(RectU8::of(0, 0, 10, 10)));
        assert_eq!(
            try_saturating_resize(&RectU8::of(u8::MAX - 2, u8::MAX - 2, u8::MAX, u8::MAX), 11),
            Some(RectU8::of(u8::MAX - 10, u8::MAX - 10, u8::MAX, u8::MAX))
        );
    }

    #[test]
    fn try_saturating_resize_even_small_rect_out_of_bounds() {
        assert_eq!(try_saturating_resize(&RectU8::of(0, 0, 3, 3), 11), Some(RectU8::of(0, 0, 10, 10)));
        assert_eq!(
            try_saturating_resize(&RectU8::of(u8::MAX - 3, u8::MAX - 3, u8::MAX, u8::MAX), 11),
            Some(RectU8::of(u8::MAX - 10, u8::MAX - 10, u8::MAX, u8::MAX))
        );
    }

    #[test]
    fn try_saturating_resize_odd_small_rect_limits_out_of_bounds() {
        assert_eq!(try_saturating_resize(&RectU8::of(0, 0, 2, 2), u8::MAX), Some(RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1)));
        assert_eq!(try_saturating_resize(&RectU8::of(u8::MAX - 2, u8::MAX - 2, u8::MAX, u8::MAX), u8::MAX), Some(RectU8::of(1, 1, u8::MAX, u8::MAX)));
    }

    #[test]
    fn try_saturating_resize_even_small_rect_limits_out_of_bounds() {
        assert_eq!(try_saturating_resize(&RectU8::of(0, 0, 3, 3), u8::MAX - 1), Some(RectU8::of(0, 0, u8::MAX - 2, u8::MAX - 2)));
        assert_eq!(
            try_saturating_resize(&RectU8::of(u8::MAX - 3, u8::MAX - 3, u8::MAX, u8::MAX), u8::MAX - 1),
            Some(RectU8::of(2, 2, u8::MAX, u8::MAX))
        );
    }

    #[test]
    fn try_saturating_resize_big_rect_limits_out_of_bounds() {
        assert_eq!(try_saturating_resize(&RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1), u8::MAX), Some(RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1)));
        assert_eq!(try_saturating_resize(&RectU8::of(1, 1, u8::MAX, u8::MAX), u8::MAX), Some(RectU8::of(1, 1, u8::MAX, u8::MAX)));
        assert_eq!(try_saturating_resize(&RectU8::largest(), u8::MAX), Some(RectU8::of(0, 0, u8::MAX - 1, u8::MAX - 1)));
    }

    #[test]
    fn saturating_resize_assign_odd() {
        let mut r = RectU8::of(5, 5, 15, 15);
        saturating_resize_assign(&mut r, 11);
        assert_eq!(r, RectU8::of(5, 5, 15, 15));
        saturating_resize_assign(&mut r, 9);
        assert_eq!(r, RectU8::of(6, 6, 14, 14));
        saturating_resize_assign(&mut r, 7);
        assert_eq!(r, RectU8::of(7, 7, 13, 13));
        saturating_resize_assign(&mut r, 5);
        assert_eq!(r, RectU8::of(8, 8, 12, 12));
        saturating_resize_assign(&mut r, 3);
        assert_eq!(r, RectU8::of(9, 9, 11, 11));
        saturating_resize_assign(&mut r, 9);
        assert_eq!(r, RectU8::of(6, 6, 14, 14));
    }

    #[test]
    fn saturating_resize_assign_even() {
        let mut r = RectU8::of(5, 5, 14, 14);
        saturating_resize_assign(&mut r, 10);
        assert_eq!(r, RectU8::of(5, 5, 14, 14));
        saturating_resize_assign(&mut r, 8);
        assert_eq!(r, RectU8::of(6, 6, 13, 13));
        saturating_resize_assign(&mut r, 6);
        assert_eq!(r, RectU8::of(7, 7, 12, 12));
        saturating_resize_assign(&mut r, 4);
        assert_eq!(r, RectU8::of(8, 8, 11, 11));
        saturating_resize_assign(&mut r, 8);
        assert_eq!(r, RectU8::of(6, 6, 13, 13));
    }

    #[test]
    fn saturating_resize_odd() {
        assert_eq!(saturating_resize(&RectU8::of(5, 5, 15, 15), 11), RectU8::of(5, 5, 15, 15));
        assert_eq!(saturating_resize(&RectU8::of(5, 5, 15, 15), 9), RectU8::of(6, 6, 14, 14));
        assert_eq!(saturating_resize(&RectU8::of(6, 6, 14, 14), 7), RectU8::of(7, 7, 13, 13));
        assert_eq!(saturating_resize(&RectU8::of(7, 7, 13, 13), 5), RectU8::of(8, 8, 12, 12));
        assert_eq!(saturating_resize(&RectU8::of(8, 8, 12, 12), 3), RectU8::of(9, 9, 11, 11));
        assert_eq!(saturating_resize(&RectU8::of(9, 9, 11, 11), 9), RectU8::of(6, 6, 14, 14));
    }

    #[test]
    fn saturating_resize_even() {
        assert_eq!(saturating_resize(&RectU8::of(5, 5, 14, 14), 10), RectU8::of(5, 5, 14, 14));
        assert_eq!(saturating_resize(&RectU8::of(5, 5, 14, 14), 8), RectU8::of(6, 6, 13, 13));
        assert_eq!(saturating_resize(&RectU8::of(6, 6, 13, 13), 6), RectU8::of(7, 7, 12, 12));
        assert_eq!(saturating_resize(&RectU8::of(7, 7, 12, 12), 4), RectU8::of(8, 8, 11, 11));
        assert_eq!(saturating_resize(&RectU8::of(8, 8, 11, 11), 8), RectU8::of(6, 6, 13, 13));
    }
}
