...
fn small_rect_limits_out_of_bounds() {
    ...
    assert_eq!(r_min_row, RectU8::of(u8::MAX / 2 + 3, u8::MAX / 2 + 5, u8::MAX / 2 + 1, u8::MAX / 2 + 3));

    let mut r_min_y = RectU8::of(2, 0, 4, 2);
    assert_eq!(try_wrapping_resize_assign(&mut r_min_y, u8::MAX), Some(()));
    assert_eq!(r_min_y, RectU8::of(u8::MAX / 2 + 5, u8::MAX / 2 + 3, u8::MAX / 2 + 3, u8::MAX / 2 + 1));

    let mut r_max_x = RectU8::of(u8::MAX - 2, u8::MAX - 4, u8::MAX, u8::MAX - 2);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_x, u8::MAX), Some(()));
    assert_eq!(r_max_x, RectU8::of(u8::MAX / 2, u8::MAX / 2 - 2, u8::MAX / 2 - 2, u8::MAX / 2 - 4));

    let mut r_max_y = RectU8::of(u8::MAX - 4, u8::MAX - 2, u8::MAX - 2, u8::MAX);
    assert_eq!(try_wrapping_resize_assign(&mut r_max_y, u8::MAX), Some(()));
    assert_eq!(r_max_y, RectU8::of(u8::MAX / 2 - 2, u8::MAX / 2, u8::MAX / 2 - 4, u8::MAX / 2 - 2));
}
