...
mod tests {
    ...
    fn bounds() {
        let cam = Cam::largest();
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(0, 0), &cam), MatrixPoint::of(u64::MAX, 0));
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(u64::MAX, 0), &cam), MatrixPoint::max());
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(0, u64::MAX), &cam), MatrixPoint::of(0, 0));
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::max(), &cam), MatrixPoint::of(0, u64::MAX));
    }

    #[test]
    fn sequence_min() {
        let cam = Cam::largest();
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(1, 1), &cam), MatrixPoint::of(u64::MAX - 1, 1));
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(2, 2), &cam), MatrixPoint::of(u64::MAX - 2, 2));
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(3, 3), &cam), MatrixPoint::of(u64::MAX - 3, 3));
    }

    #[test]
    fn sequence_max() {
        let cam = Cam::largest();
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(u64::MAX - 1, u64::MAX - 1), &cam), MatrixPoint::of(1, u64::MAX - 1));
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(u64::MAX - 2, u64::MAX - 2), &cam), MatrixPoint::of(2, u64::MAX - 2));
        assert_eq!(cartesian_in_cam_to_matrix(&CartesianPoint::of(u64::MAX - 3, u64::MAX - 3), &cam), MatrixPoint::of(3, u64::MAX - 3));
    }
}
