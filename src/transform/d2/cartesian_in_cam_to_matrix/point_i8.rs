...
mod tests {
    ...
    fn bounds() {
        let cam = Cam::largest();
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::min(), &cam), Matrix::new(u8::MAX, 0));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(i8::MAX, i8::MIN), &cam), Matrix::max());
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(i8::MIN, i8::MAX), &cam), Matrix::min());
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::max(), &cam), Matrix::new(0, u8::MAX));
    }

    #[test]
    fn sequence_min() {
        let cam = Cam::largest();
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(i8::MIN + 1, i8::MIN + 1), &cam), Matrix::new(u8::MAX - 1, 1));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(i8::MIN + 2, i8::MIN + 2), &cam), Matrix::new(u8::MAX - 2, 2));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(i8::MIN + 3, i8::MIN + 3), &cam), Matrix::new(u8::MAX - 3, 3));
    }

    #[test]
    fn sequence_max() {
        let cam = Cam::largest();
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(i8::MAX - 1, i8::MAX - 1), &cam), Matrix::new(1, u8::MAX - 1));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(i8::MAX - 2, i8::MAX - 2), &cam), Matrix::new(2, u8::MAX - 2));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(i8::MAX - 3, i8::MAX - 3), &cam), Matrix::new(3, u8::MAX - 3));
    }
}
