...
mod tests {
    ...
    fn bounds() {
        ...
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(u64::MAX, 0), &cam), Matrix::max());
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(0, u64::MAX), &cam), Matrix::min());
    ...
    fn sequence_min() {
        let cam = Cam::largest();
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(1, 1), &cam), Matrix::new(u64::MAX - 1, 1));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(2, 2), &cam), Matrix::new(u64::MAX - 2, 2));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(3, 3), &cam), Matrix::new(u64::MAX - 3, 3));
    }

    #[test]
    fn sequence_max() {
        let cam = Cam::largest();
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(u64::MAX - 1, u64::MAX - 1), &cam), Matrix::new(1, u64::MAX - 1));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(u64::MAX - 2, u64::MAX - 2), &cam), Matrix::new(2, u64::MAX - 2));
        assert_eq!(cartesian_in_cam_to_matrix(&Cartesian::new(u64::MAX - 3, u64::MAX - 3), &cam), Matrix::new(3, u64::MAX - 3));
    }
}
