...
mod tests {
    ...
    fn edges() {
        let cam = Cam::largest();
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::min(), &cam), Cartesian::new(0, u8::MAX));
        ...
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::max(), &cam), Cartesian::new(u8::MAX, 0));
    }

    #[test]
    fn sequence_min() {
        let cam = Cam::largest();
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::new(1, 1), &cam), Cartesian::new(1, u8::MAX - 1));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::new(2, 2), &cam), Cartesian::new(2, u8::MAX - 2));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::new(3, 3), &cam), Cartesian::new(3, u8::MAX - 3));
    }

    #[test]
    fn sequence_max() {
        let cam = Cam::largest();
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::new(u8::MAX - 1, u8::MAX - 1), &cam), Cartesian::new(u8::MAX - 1, 1));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::new(u8::MAX - 2, u8::MAX - 2), &cam), Cartesian::new(u8::MAX - 2, 2));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::new(u8::MAX - 3, u8::MAX - 3), &cam), Cartesian::new(u8::MAX - 3, 3));
    }
}
