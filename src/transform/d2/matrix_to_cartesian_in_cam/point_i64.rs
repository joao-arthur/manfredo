...
mod tests {
    ...
    fn edges() {
        let cam = Cam::largest();
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::min(), &cam), Cartesian::new(i64::MIN, i64::MAX));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(u64::MAX, 0), &cam), Cartesian::min());
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(0, u64::MAX), &cam), Cartesian::max());
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::max(), &cam), Cartesian::new(i64::MAX, i64::MIN));
    }

    #[test]
    fn sequence_min() {
        let cam = Cam::largest();
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(1, 1), &cam), Cartesian::new(i64::MIN + 1, i64::MAX - 1));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(2, 2), &cam), Cartesian::new(i64::MIN + 2, i64::MAX - 2));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(3, 3), &cam), Cartesian::new(i64::MIN + 3, i64::MAX - 3));
    }

    #[test]
    fn sequence_max() {
        let cam = Cam::largest();
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(u64::MAX - 1, u64::MAX - 1), &cam), Cartesian::new(i64::MAX - 1, i64::MIN + 1));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(u64::MAX - 2, u64::MAX - 2), &cam), Cartesian::new(i64::MAX - 2, i64::MIN + 2));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(u64::MAX - 3, u64::MAX - 3), &cam), Cartesian::new(i64::MAX - 3, i64::MIN + 3));
    }
}
