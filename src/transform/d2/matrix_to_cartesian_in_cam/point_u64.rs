...
mod tests {
    ...
    fn edges() {
        let cam = Cam::largest();
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::min(), &cam), Cartesian::new(0, u64::MAX));
        ...
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::max(), &cam), Cartesian::new(u64::MAX, 0));
    }

    #[test]
    fn sequence_min() {
        let cam = Cam::largest();
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(1, 1), &cam), Cartesian::new(1, u64::MAX - 1));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(2, 2), &cam), Cartesian::new(2, u64::MAX - 2));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(3, 3), &cam), Cartesian::new(3, u64::MAX - 3));
    }

    #[test]
    fn sequence_max() {
        let cam = Cam::largest();
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(u64::MAX - 1, u64::MAX - 1), &cam), Cartesian::new(u64::MAX - 1, 1));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(u64::MAX - 2, u64::MAX - 2), &cam), Cartesian::new(u64::MAX - 2, 2));
        assert_eq!(matrix_to_cartesian_in_cam(&Matrix::new(u64::MAX - 3, u64::MAX - 3), &cam), Cartesian::new(u64::MAX - 3, 3));
    }
}
