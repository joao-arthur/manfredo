...
mod tests {
    ...
    fn edges() {
        let cam = Cam::largest();
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(0, 0), &cam), Cartesian::of(0, u64::MAX));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(u64::MAX, 0), &cam), Cartesian::of(0, 0));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(0, u64::MAX), &cam), Cartesian::max());
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::max(), &cam), Cartesian::of(u64::MAX, 0));
    }

    #[test]
    fn sequence_min() {
        let cam = Cam::largest();
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(1, 1), &cam), Cartesian::of(1, u64::MAX - 1));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(2, 2), &cam), Cartesian::of(2, u64::MAX - 2));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(3, 3), &cam), Cartesian::of(3, u64::MAX - 3));
    }

    #[test]
    fn sequence_max() {
        let cam = Cam::largest();
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(u64::MAX - 1, u64::MAX - 1), &cam), Cartesian::of(u64::MAX - 1, 1));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(u64::MAX - 2, u64::MAX - 2), &cam), Cartesian::of(u64::MAX - 2, 2));
        assert_eq!(matrix_in_cam_to_cartesian(&Matrix::of(u64::MAX - 3, u64::MAX - 3), &cam), Cartesian::of(u64::MAX - 3, 3));
    }
}
